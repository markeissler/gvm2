...
_gvm()
{
	...
	case "${cmd}" in
		pkgset)
			case "${subcmd}" in
				use|delete|empty)
					...
					local version="$(for x in `\ls -1 $GVM_ROOT/pkgsets/$gvm_go_name`; do echo ${x} ; done )"
			...
			case "${subcmd}" in
				uninstall)
					...
					if [[ "$COMP_CWORD" == "3" ]]; then
						local version="$(for x in `\ls -1 $GVM_ROOT/pkgsets/$gvm_go_name/$gvm_pkgset_name/pkg.gvm`; do echo ${x} ; done )"
						COMPREPLY=( $(compgen -W "${version}" -- ${cur}) )
						return 0
					elif [[ "$COMP_CWORD" == "4" ]]; then
						local version="$(for x in `\ls -1 $GVM_ROOT/pkgsets/$gvm_go_name/$gvm_pkgset_name/pkg.gvm/${COMP_WORDS[3]}`; do echo ${x} ; done )"
						COMPREPLY=( $(compgen -W "${version}" -- ${cur}) )
						return 0
					fi
					return 1
		...
		install)
			...
			local version="$(builtin cd $GVM_ROOT/archive/go && for x in `git tag | \grep -E "release|tip|go"`; do echo ${x} ; done )"
			COMPREPLY=( $(compgen -W "${version}" -- ${cur}) )
			return 0
		;;
		use|uninstall)
			...
			[[ $COMP_CWORD > 2 ]] && return 1
			local installed_versions="$(for x in `\ls -1 $GVM_ROOT/gos`; do echo ${x} ; done )"
