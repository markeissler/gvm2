...
##
## find all source packages available for linux
##
## 1
## 1.3
...
## Evaluate result
for version in "${availableVersions[@]}";do [[ "${version}" == "go1" ]] && break; done # status=0
for version in "${availableVersions[@]}";do [[ "${version}" == "go1.2" ]] && break; done # status=0
...
##
## find all binary packages available for linux
##
## 1.2
## 1.3
## 1.4
